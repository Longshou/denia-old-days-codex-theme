...
让 Codex 原生右侧栏从约 `320px` 的常规宽度拖拽到全工作区宽度时，人物近景自然过渡为完整舞台场景；同时消除快捷项首帧白闪、选项外框和底部输入框的外溢焦点线。
...
- 宽度介于 `440px` 和 `840px`。
- 进度为 `(width - 440) / 400`，约束在 `0` 到 `1`。
- 近景不透明度为 `1 - progress`。
- 舞台环境层和完整场景层不透明度随 `progress` 增加。
- 只过渡 `opacity` 和轻微 `transform`，不动画宽度、高度和布局。
...
- 观察回调只安排现有侧栏刷新，不直接循环读取和写入布局。
- 每次同步设置面板局部变量 `--denia-sidebar-wide-progress`。
...
- 不修改输入框宽度、高度、圆角布局、工具栏位置、快捷键和提交行为。
...
- 浅色和深色模式均检查 `320px` 窄栏。
- 拖拽经过约 `560px` 和 `760px` 时画面连续，无白闪和跳图。
