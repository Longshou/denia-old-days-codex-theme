...
background-color 值、且直接挂载或移除于 body 的探针节点；普通或嵌套区域的同形节点仍按语义变更处理。
...
最终策略下连续三轮主题探针的完整刷新次数中位数为：

- 侧栏打开 2 次、关闭 3 次。
- 底部面板打开 1 次、关闭 2 次。

每格按“完整刷新 / 最大帧间隔毫秒 / 50 毫秒长任务数”记录：

| 探针 | 侧栏开 | 侧栏关 | 底部开 | 底部关 | 恢复状态 |
| --- | --- | --- | --- | --- | --- |
| 主题 1 | 1 / 25.0 / 0 | 2 / 28.9 / 0 | 1 / 50.9 / 1 | 2 / 33.2 / 0 | 全部 closed |
| 主题 2 | 3 / 49.5 / 0 | 4 / 41.6 / 0 | 2 / 325.0 / 2 | 2 / 42.3 / 0 | 全部 closed |
| 主题 3 | 2 / 43.5 / 0 | 3 / 44.7 / 2 | 1 / 275.1 / 3 | 2 / 71.5 / 0 | 全部 closed |
| 原生基线 | 0 / 43.5 / 0 | 0 / 31.9 / 0 | 0 / 327.1 / 3 | 0 / 60.9 / 0 | 全部 closed |

最后一次安装后的复测为 1/2/1/2，四个方向均不超过 5 次，终端 composer 误装饰始终为 0。
原生基线的侧栏开关没有 50 毫秒长任务；最终主题复测同样为 0。底部首次打开的原生基线包含
约 326 毫秒终端初始化长任务，主题复测没有增加长任务数量。

布局回归中，侧栏关闭时内容 x 坐标从约 332 单调收敛到 340.8，没有先向右冲到底再回弹；
置顶摘要打开两轮时，主栏候选容器的原生左右边距均为 237px。
