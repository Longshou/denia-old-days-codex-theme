...
- **每个状态只保留一个主体。** approval 的横版海报裁切出现多个视觉焦点，接近拼贴。working/complete 使用一张布景之形单人透明图；approval/error 使用同一张幻灭之形单人裁切。状态变化只改色调、明度和透明度，不换构图位置。

### P1

- working 与 complete 不能空栏。两态都应显示低对比布景之形，维持角色陪伴感和布局稳定。
...
- 单一 `::after` 不足以完成双形态交叉淡化。增加固定尺寸的 light-form 与 dark-form 两层，两个图层使用同一锚点和裁切盒。
...
| 状态 | 侧栏美术 | 建议透明度 | 色调与情绪 |
| --- | --- | ---: | --- |
| staged | 无人物，保留纸张底纹 | 0 | 中性、安静 |
| working | 布景之形，固定半身构图 | .14-.18 | 冷青、专注 |
| approval | 幻灭之形，同位构图 | .38-.46 | 低饱和紫、悬停 |
| error | 幻灭之形，同一构图 | .52-.60 | 深紫加窄幅洋红、警觉 |
| complete | 布景之形，回到原锚点 | .18-.22 | 暖金高光、收束 |
...
- **working：** 布景之形单人半身或透明立绘，视线朝向工作区，冷青单色化，透明度 `.14-.18`。不新增文字，状态文字仍由观察区和 pill 承担。
- **complete：** 同一布景之形、同一位置，透明度 `.18-.22`，加轻微暖金高光。保持静态，不使用庆祝粒子或循环呼吸。
...
- 创建一次 `.denia-old-days-ds-state-art`，内部只有 `light` 与 `dark` 两个 `span`；图片固定，不在每次状态变化时重建节点。
- `refresh()` 只更新 `data-denia-form-state`、状态文案和两层目标透明度，不再切换 `background-position`。
